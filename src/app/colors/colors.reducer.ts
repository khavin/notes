...
export const colorReducer = createReducer(
    ...
    on(addColor, (state, { color }) => {
        if(color.toUpperCase() in colorDefinitions){
            if(state.noColorSelected == true){
                ...
                let tempColors:Array<string> = state.pickedColors.filter(pickedColor => pickedColor != color.toUpperCase());
    ...
    on(removeColor, (state,{color}) => {
        if(color.toUpperCase() in colorDefinitions && state.noColorSelected == false && state.pickedColors.includes(color.toUpperCase())){
            let tempColors:Array<string> = state.pickedColors.filter(pickedColor => pickedColor != color.toUpperCase());
